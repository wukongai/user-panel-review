...
- 在 Windows 或默认输出编码不是 UTF-8 的宿主上运行 `user_review.py`。
...
仓库外测试 `tests/test_user_review_v03.py` 必须在模拟非 UTF-8 默认编码时验证上述错误输出；GitHub Actions 必须覆盖 Windows、macOS 与 Linux。
