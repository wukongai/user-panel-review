...
安装包已经准备了一篇示例文章和几类内置示例用户。内置演示选择的是 AI 文章，只是为了让新用户安装后立即看到反馈效果，不代表 Skill 只适用于 AI 内容。第一次不用设置任何东西，直接对 Agent 说：
...
这篇文章可以是教育、心理、美妆、职场、品牌、课程或其他主题。当前正式回归的是“文章阅读反馈”流程，文章属于什么领域没有限制。
...
用户不需要告诉 Agent 应该怎样提问。Agent 会先复述已经知道的信息，然后自动一次只确认一个真正会改变用户分层的关键问题，例如：
...
回答不需要专业。像平时介绍自己的业务一样说即可，任何领域都可以，例如：

```text
我给高中学生做学习方法内容，也会给家长提供选课建议。
```
...
下面是一条教育场景的真实连续测试。示例用户只说明自己面向高中学生，也为家长和老师提供内容；Agent 自动确认业务主线，并把三类角色整理成不同的目标用户。用户不需要规定提问方式，只需像平时介绍业务一样回答，并在候选不准确时直接修改：
...
下面这次真实交互继续使用刚刚保存的四类教育目标用户，让他们反馈《三段十分钟学习法》。用户确认时只说“确认开始，请反馈这篇文章”，Agent 就自动使用默认的五个栏目：
...
同一条教育场景的真实对话中，Agent 会先解释这类人会多看什么、少看什么；确认后只补充差异反馈，并再次确认长期用户仍是原来的四类：
